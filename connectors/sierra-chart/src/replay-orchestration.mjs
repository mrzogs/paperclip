...
export function extractReplayStudySettings(messageEvents = []) {
  ...
  return {
    ...
    targetStopRatchetTriggerSlot: bracketPlan?.payload?.target_stop_ratchet_trigger_slot ? Number(bracketPlan.payload.target_stop_ratchet_trigger_slot) : null,
    targetStopRatchetStopSlot: bracketPlan?.payload?.target_stop_ratchet_stop_slot ? Number(bracketPlan.payload.target_stop_ratchet_stop_slot) : null,
...
export function verifyReplayStudySettings(actualSettings, expectedSettings = {}) {
  const comparisons = {
    ...
    targetStopRatchetTriggerSlot: compareExpectedSetting(actualSettings?.targetStopRatchetTriggerSlot, expectedSettings.targetStopRatchetTriggerSlot),
    targetStopRatchetStopSlot: compareExpectedSetting(actualSettings?.targetStopRatchetStopSlot, expectedSettings.targetStopRatchetStopSlot),
...
function makeReplayTradeFromBracketEvent(event, profileByBarIndex, sourceFile, dedupe) {
  ...
  return {
    ...
    context: {
      ...
      targetStopRatchetTriggerSlot: event.payload.target_stop_ratchet_trigger_slot ? Number(event.payload.target_stop_ratchet_trigger_slot) : null,
      targetStopRatchetStopSlot: event.payload.target_stop_ratchet_stop_slot ? Number(event.payload.target_stop_ratchet_stop_slot) : null,
